...
class TestClass(object):
    def _createAssetNode(self):
        ...
        result = [
            record["node"] for record in graphDB.createAssetNode(
                Asset(exchange='Bitfinex', symbol='BTC'))
        ]
        result_validation = [
            record["node"] for record in graphDB.runCypher(
                "MATCH (n) WHERE n.exchange='Bitfinex' AND n.symbol='BTC' RETURN id(n) AS node"
            )
        ]
