...
class Trader:
    ...
    def __init__(self, exchangeNames: List[str] = [], credfile='./cred/api.json', isSandboxMode=True):
        self.keys = {}
        self.balance = {}
    ...
    async def cancelTradeOrder(self, trade: Trade):
        for retrycntr in range(Trader.NOF_CCTX_RETRY):
            try:
                response = await self.exchanges[trade.exchangeNameStd].cancelOrder(trade.id)
                if response['error']:
                ...
                logger.error('Trader order cancellation failed for ' + str(
                    trade.id) + " " + trade.market + " " + trade.exchangeName + " " + str(e.args) + " retrycntr:" + str(
                    retrycntr))
                await asyncio.sleep(self.exchanges[trade.exchangeNameStd].rateLimit / 1000)
    ...
    async def fetchOrderStatus(self, trade: Trade):
        for retrycntr in range(Trader.NOF_CCTX_RETRY):
            try:
                response = await self.exchanges[trade.exchangeNameStd].fetchOrder(trade.id)
                ...
                logger.error('Order status fetching failed for ' + str(
                    trade.id) + " " + trade.market + " " + trade.exchangeName + " " + str(e.args) + " retrycntr:" + str(
                    retrycntr))
                await asyncio.sleep(self.exchanges[trade.exchangeNameStd].rateLimit / 1000)
    ...
    async def fetch_balance(self, exchange):
        for retrycntr in range(Trader.NOF_CCTX_RETRY):
            try:
                ...
                logger.error('Fetch balance failed from ' + exchange.name + " " + type(error).__name__ + " " + str(
                    error.args) + " retrycntr:" + str(retrycntr))
    ...
    def get_exchange(self, exchange_name: str) -> Exchange:
        if exchange_name not in self.exchanges:
            raise ValueError(f'Exchange ({exchange_name}) does not exists in initialized exchanges')
        return self.exchanges[exchange_name]

    def get_market(self, exchange_name: str, market_str: str) -> Dict[str, Any]:
        exchange = self.get_exchange(exchange_name)
        if market_str not in exchange.markets:
            raise ValueError(f'Symbol ({market_str}) does not exists in exchange ({exchange_name})')
    ...
    def is_transaction_valid(self, exchange_name: str, market_str: str, amount: float):
        ...
        if market['limits']['price']['min']:
            if amount < exchange.markets[market_str]['limits']['price']['min']:
                raise ValueError(
                    'Amount too small, won''t execute on ' + exchange.name + " " + market_str + " Amount: " + str(
                        amount) +
                    " Min.amount:" + str(exchange.markets[market_str]['limits']['price']['min']))

        if market['limits']['price']['max']:
            if amount > exchange.markets[market_str]['limits']['price']['max']:
                raise ValueError(
                    'Amount too big, won''t execute on ' + exchange.name + " " + market_str + " Amount: " + str(
                        amount) +
                    " Max.amount:" + str(exchange.markets[market_str]['limits']['price']['max']))

        if self.get_free_balance(exchange_name, market_str.split('/')[0]) < amount:
            raise ValueError('Insufficient stock on ' + exchange.name + " " + market_str + " Amount available: " + str(
                self.get_free_balance(exchange_name, market_str)) +
                             " Amount required:" + str(amount))

        return True

    def is_trade_valid(self, trade: Trade):
        return self.is_transaction_valid(trade.exchangeNameStd, trade.market, trade.amount)
    ...
    async def create_limit_order(self, trade: Trade):
        ...
        try:
            ...
            if tradetype == TradeType.BUY:
                ...
                logger.info(
                    "createLimitBuyOrder " + symbol + " Amount:" + str(amount) + " Price:" + str(price) + " ID: " +
                ...
                logger.info(
                    "createLimitSellOrder " + symbol + " Amount:" + str(amount) + " Price:" + str(price) + " ID: " +
                    response['id'] + " created successfully")
            else:
            ...
            logger.error('createLimitBuyOrder failed from ' + exchange.name + " " + symbol + ": " + type(
                error).__name__ + " " + str(error.args))
    ...
    def create_limit_orders(self, tradelist: List[Trade]):
        ...
        try:
            ...
            for trade in tradelist:
                orders.append(asyncio.ensure_future(self.create_limit_order(trade)))
