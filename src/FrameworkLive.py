...
class FrameworkLive:
    def __init__(self,isForexEnabled=False,enablePlotting=False, isSandboxMode=True,resultsdir="./"):
        ...
        self.symbols['coinfloor'] =['BTC/EUR','BTC/EUR','BCH/GBP','BTC/GBP','BTC/USD']
        self.symbols['kraken'] =['BTC/USD','BTC/EUR','BTC/USD','BCH/USD','XRP/USD','LTC/EUR','LTC/USD','ETH/BTC','BCH/BTC','XRP/BTC']
        self.symbols['bittrex'] =['XRP/BTC','BTC/USD','BCH/BTC','BTC/USD','LTC/BTC','XRP/ETH','BCH/ETH','ETH/BTC','LTC/ETH','BCH/USDT']
        self.symbols['gdax'] =['BCH/BTC','BTC/EUR','LTC/EUR','BTC/USD','BTC/EUR','ETH/USD','ETH/EUR','BCH/EUR','ETH/BTC','BCH/USD']
        self.symbols['bitstamp'] =['BTC/EUR','ETH/BTC','BTC/USD','ETH/USD','BCH/EUR','BCH/BTC','LTC/EUR','ETH/EUR','XRP/BTC','LTC/BTC']
        self.symbols['poloniex'] =['BCH/BTC','XRP/BTC','LTC/BTC','ETH/BTC','BCH/ETH','ETC/BTC','ETC/ETH','LSK/ETH','LSK/BTC','LTC/USDT']
    ...
    async def forexPoller(self,symbols,authkey,orderbookAnalyser):
        async for ticker in self.pollForex(symbols=symbols,authkey=authkey):
            logger.info("Received "+ticker['prices'][0]['instrument'].replace("_","/") +" prices from Oanda")
    ...
    async def pollCoinmarketcap(self,cmc):
        ...
        while True:
            ...
            try:
                yield (await cmc.fetch_tickers(symbol_quote,params={"limit":40,"sort":"rank"}))
            except (ccxt.ExchangeError, ccxt.NetworkError) as error:
    ...
    def run(self):
        ...
        Thread(target=self.consumeArbTradeTriggerEvent, args=(self.arbTradeTriggerEvent,self.arbTradeQueue,self.isSandboxMode)).start()
        loop.run_forever()
        
        self.orderbookAnalyser.generateExportFilename(list(self.exchanges.keys()))
